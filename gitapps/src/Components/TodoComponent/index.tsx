...
function TodoApp() {
  ...
  function removeTask(id: string, isEditing: boolean, task: string) {
    ...
    setFinishedTask([
      ...finishedTask,
      { id: id, isEditing: !isEditing, $finished: true, task: task },
    ]);
  }
  ...
  function UpdateTask(idToUpdate: string, isEditing: boolean) {
    ...
    setTaskItem((prevState) =>
      prevState.map((obj) =>
        obj.id == idToUpdate
          ? { ...obj, task: updatedTask.editTask, isEditing: !isEditing }
          : obj
      )
    );
    resetField("editTask");
    ...
    <TodoContainer>
      ...
      <ContentArea>
        {errors?.task?.type === "required" && (
          <p style={{ color: "red" }}>Preencha o campo "Tarefa"</p>
        )}
        {taskItem.length
          ? taskItem.map((item, index) => (
              <TaskArea key={index} onClick={() => console.log(taskItem)}>
                ...
                <div>
                  <FinishTaskButton
                    onClick={() =>
                      removeTask(item.id, item.isEditing, item.task)
                    }
                  >
                    Concluir
                  </FinishTaskButton>
                  {item.isEditing ? (
                    <EditTaskButton
                      onClick={() => UpdateTask(item.id, item.isEditing)}
                    >
                      Salvar
                    </EditTaskButton>
                  ) : (
